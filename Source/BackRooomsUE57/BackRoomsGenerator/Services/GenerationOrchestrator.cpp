...
int32 FGenerationOrchestrator::ExecuteProceduralGeneration(
    ...
    TFunction<void(FRoomData&)> RoomCreator)
{
    ...
    LogDebug(FString::Printf(TEXT("=== Generating %d units: %.1f%% rooms, %.1f%% hallways, %.1f%% stairs ==="), 
        Config.TotalRooms, Config.RoomRatio * 100.0f, Config.HallwayRatio * 100.0f, Config.StairRatio * 100.0f), Config);
    ...
    for (int32 RoomIndex = 1; RoomIndex < Config.TotalRooms && AvailableRooms.Num() > 0 && !bEmergencyExit; RoomIndex++)
    {
        ...
        LogDebug(FString::Printf(TEXT("🏗️  GENERATING ROOM %d/%d (MainLoop: %d/%d)"), 
            RoomIndex, Config.TotalRooms-1, MainLoopCounter, Config.MaxSafetyIterations), Config);
        LogDebug(TEXT("================================================================================"), Config);
        ...
        while (!bRoomPlaced && ConnectionRetries < Config.MaxConnectionRetries && AvailableRooms.Num() > 0 && !bEmergencyExit)
        {
            ...
            {
                LogDebug(FString::Printf(TEXT("Room %d has no available connections, will try other rooms"), RandomRoomIndex), Config);
            ...
            if (TryGenerateConnectedRoom(RoomIndex, SourceRoom, ConnectionIndex, OutGeneratedRooms, 
                                       Config, CollisionService, ConnectionManager, RoomCreator, Random))
            {
                bRoomPlaced = true;
                ...
                ConnectionManager->ConnectRooms(OutGeneratedRooms[RandomRoomIndex], ConnectionIndex, 
                                             OutGeneratedRooms[RoomIndex], OppositeConnectionIndex, Config);
                
                LogDebug(FString::Printf(TEXT("✅ ROOM %d PLACED: %s connected to room %d"), 
                    RoomIndex, *UEnum::GetValueAsString(OutGeneratedRooms[RoomIndex].Category), RandomRoomIndex), Config);
            }
            else
            {
                LogDebug(FString::Printf(TEXT("❌ Failed to place room %d at connection %d, retrying..."), 
                    RoomIndex, ConnectionIndex), Config);
            }
        }
        ...
        if (!bRoomPlaced)
        {
            LogDebug(FString::Printf(TEXT("❌ ROOM %d: Could not place after %d retries"), 
                RoomIndex, ConnectionRetries), Config);
        }
    ...
    LogDebug(FString::Printf(TEXT("✅ GENERATION COMPLETED: %d/%d rooms generated"), 
        OutGeneratedRooms.Num(), Config.TotalRooms), Config);
    ...
    LogDebug(FString::Printf(TEXT("🔄 Loop counters: Main=%d, Connection=%d, Placement=%d"), 
        MainLoopCounter, ConnectionRetryCounter, PlacementAttemptCounter), Config);
    if (bStoppedBySafety)
    {
...
void FGenerationOrchestrator::GetGenerationStats(int32& OutMainLoops, int32& OutConnectionRetries, 
...
bool FGenerationOrchestrator::CheckSafetyLimits(const FBackroomGenerationConfig& Config)
{
    ...
    {
        LogDebug(FString::Printf(TEXT("⏰ TIME LIMIT REACHED: Stopping generation after %.1f seconds"), ElapsedTime), Config);
...
TPair<ERoomCategory, FString> FGenerationOrchestrator::DetermineRoomCategory(
...
bool FGenerationOrchestrator::TryGenerateConnectedRoom(
    int32 RoomIndex,
    const FRoomData& SourceRoom,
    ...
    FRandomStream& Random)
{
    ...
    for (int32 Attempt = 0; Attempt < Config.MaxAttemptsPerConnection; Attempt++)
    {
        ...
        LogDebug(FString::Printf(TEXT("Placement attempt %d/%d for connection %d"), 
            Attempt+1, Config.MaxAttemptsPerConnection, ConnectionIndex), Config);
        ...
        FRoomData NewRoom = GenerateRoomOfCategory(Category, RoomIndex, 
            Category == ERoomCategory::Stairs ? &SourceRoom : nullptr, ConnectionIndex);
        LogDebug(FString::Printf(TEXT("Generated %s: %.1fx%.1fm"), *CategoryStr, NewRoom.Width, NewRoom.Length), Config);
        
        // Try to place the room
        LogDebug(FString::Printf(TEXT("Attempting to place room at connection...")), Config);
        if (ConnectionManager->TryPlaceRoom(SourceRoom, ConnectionIndex, NewRoom, OutGeneratedRooms, 
                                          Config, CollisionService, RoomCreator))
        {
...
FRoomData FGenerationOrchestrator::GenerateRoomOfCategory(ERoomCategory Category, int32 RoomIndex,
                                                        const FRoomData* SourceRoom, int32 ConnectionIndex) const
{
