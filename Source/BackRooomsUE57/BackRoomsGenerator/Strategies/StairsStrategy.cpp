...
FRoomData FStairsStrategy::GenerateRoom(const FBackroomGenerationConfig& Config, int32 RoomIndex)
{
    ...
    Room.StairDirection = PossibleDirections[Random.RandRange(0, PossibleDirections.Num() - 1)];
...
bool FStairsStrategy::CanGenerateRoom(const FBackroomGenerationConfig& Config, 
                                     const FRoomData* SourceRoom) const
{
    ...
    bool bValidElevationRange = (Config.MinStairHeight > 0.0f && 
                                Config.MaxStairHeight > Config.MinStairHeight &&
                                                  ...
                                                  FRandomStream& Random) const
{
    ...
    {
        TArray<EWallSide> PossibleDirections = {EWallSide::North, EWallSide::South, EWallSide::East, EWallSide::West};
        return PossibleDirections[Random.RandRange(0, PossibleDirections.Num() - 1)];
    }
...
bool FStairsStrategy::ValidateStairsDimensions(const FBackroomGenerationConfig& Config,
                                              float Width, 
                                              float Length) const
{
